...
class User(db.Model):
    ...
    def set_password(self, password):
        """Хешування пароля та збереження його."""
...
class Asset(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)  # Адміністратор, який створив об'єкт
    name = db.Column(db.String(100), nullable=False)
    type = db.Column(db.String(50), nullable=False)  # Додано поле type
    ...
    price_per_day = db.Column(db.Float, nullable=False)  # Renamed for clarity
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
...
class Rental(db.Model):
    ...
    end_date = db.Column(db.Date, nullable=True)  # Додано поле end_date
    total_cost = db.Column(db.Float, nullable=True)  # Додано поле total_cost
    status = db.Column(db.String(50), nullable=False, default='Активний')  # Додано поле status
